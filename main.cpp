...
uint64 disMax[THREAD_COUNT_READ];
uint32 index2Id[MAXN];
uint32 nodeCnt;
uint16 bitsFlag;
uint32 disArray[THREAD_COUNT][DIS_ARRAY_LEN];
/*************************** tarjan ***************************************/
...
void tarjan() {
    ...
    clock_t end = clock();
    cout << "tarjan_time: " << (double)(end - start) / CLOCKS_PER_SEC << endl;
    cout << "scc_size: " << tarjanCnt << endl;
...
#ifdef TEST
    uint16 sortOutDegree[MAXN];
    uint16 sortInDegree[MAXN];
    uint32 oneI_zeroO = 0;
    uint32 oneO_zeroI = 0;
    for (int i = 0; i < nodeCnt; i++) {
        if (inDegree[0][i] == 1 && outDegree[0][i] == 0) oneI_zeroO++;
        if (outDegree[0][i] == 1 && inDegree[0][i] == 0) oneO_zeroI++;
    }
    memcpy(sortOutDegree, outDegree, nodeCnt * sizeof(uint16));
    memcpy(sortInDegree, inDegree, nodeCnt * sizeof(uint16));
    cout << "node_size:" << nodeCnt<< endl;
    sort(sortOutDegree, sortOutDegree+nodeCnt);
//    for (int i = 0; i < nodeCnt; i++) {
//        if (sortInDegree[i] == 1 && sortOutDegree[i] == 0) oneI_zeroO++;
//        if (sortOutDegree[i] == 0 && sortInDegree[i] == 1) oneO_zeroI++;
//    }
    cout << "oneI_zeroO: " << oneI_zeroO << endl;
    cout << " oneO_zeroI: " << oneO_zeroI << endl;
    cout << "min-out_degree:" << sortOutDegree[0] << "\t\tmax_out_degree:" << sortOutDegree[nodeCnt-1] << endl;
    sort(sortInDegree, sortInDegree+nodeCnt);
    cout << "min_in_degree:" << sortInDegree[0] << "\t\tmax_in_degree:" << sortInDegree[nodeCnt-1] << endl;
    cout << "max_money;" << moneyMax[0] << endl;
//    tarjan();
#endif
}
        ...
        while (!Q.empty()) {
            ...
            for (const uint32 &u:idList) {
                ...
                for (uint32 it = G2[u].l; it < G2[u].r; ++it) {
                    ...
                    uint32 disSum = curDis + neighborsTable2[it].w;
                    if (dis[v] > disSum) {
                        disMax[threadId] = max(disMax[threadId], (uint64)disSum);
                        dis[v] = disSum;
                        sigma[v] = sigma[u];
                        P[v][0] = 0;
                        P[v][++P[v][0]] = u;
                        ...
                        disArray[threadId][dis[v]]++;
                        piv.push_back(v);
                    } else if (dis[v] == disSum && sigma[u]) {
                        sigma[v] += sigma[u];
                        P[v][++P[v][0]] = u;
                    }
                }
        ...
        while (!Q.empty()) {
            ...
            for (const uint32 &u:idList) {
                ...
                for (uint32 it = G2[u].l; it < G2[u].r; ++it) {
                    ...
                    if (dis[v] > disSum) {
                        disMax[threadId] = max(disMax[threadId], disSum);
                        dis[v] = disSum;
                        sigma[v] = sigma[u];
                        P[v][0] = 0;
                        P[v][++P[v][0]] = u;
...
void writeResult(const string &outputFile) {
    for (uint32 i = 0; i < THREAD_COUNT; ++i) {
        ...
        for (int idx = 0; idx < nodeCnt; ++idx)
            resultGlobal[idx].bc += resultThread[idx];
    }
...
void getMaxDis() {
    for (int i = 1; i < THREAD_COUNT_READ; i++) {
        disMax[0] = max(disMax[0], disMax[i]);
    }
    cout << "max_distance:" << disMax[0] << endl;
}
void getDisDistribution() {
    cout << "get DisArray" << endl;
    uint32 maxSize = 0;
    uint32 index = 0;
    string outputFile = "/home/disDistribution.txt";
    ofstream fout(outputFile);
    for (int i = 1; i < THREAD_COUNT; i++) {
        for (int j = 0; j <= DIS_ARRAY_LEN; j++) {
            disArray[0][j] += disArray[i][j];
        }

    }
    for (int i = 1; i <= DIS_ARRAY_LEN; i++) {
        maxSize = max(maxSize, disArray[0][i]);
        if (maxSize == disArray[0][i]) index = i;
        fout << disArray[0][i] << endl;
    }
    cout << "most_dis:" << maxSize << endl;
    cout << "most_dis_index: " << index << endl;
    fout.close();
}
