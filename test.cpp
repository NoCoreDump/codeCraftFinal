...
const int MAX_NO_HASH_SIZE = MAXN*4;
...
struct Bundle {
    ...
    //indexed with distance
    vector<ui> noHashIDList[MAX_NO_HASH_SIZE];
    ui noHashDisSet[MAX_NO_HASH_SIZE];
    unordered_map<ull,vector<ui>> disMap;
//    __gnu_pbds::gp_hash_table<ull, bool> disSet;
    unordered_set<ull> disSet;

    inline bool checkDis(const ui& head,const ull &distance){
        bool ret;
        if(distance<MAX_NO_HASH_SIZE){
            ret=noHashDisSet[distance]==head;
            if(!ret) noHashDisSet[distance]=head;
        }else{
            ret=disSet.find(distance)!=disSet.end();
            if(!ret) disSet.insert(distance);
        }
        return ret;
    }

    inline vector<ui>* getIDList(const ull &distance){
        if(distance<MAX_NO_HASH_SIZE){
            return &noHashIDList[distance];
        }else{
            return &disMap[distance];
        }
    }
...
//    timer.setTime();
    ...
    for (int i = startIdx; i < endIdx; ++i) {
        ...
        while(!pq.empty()) {
            ...
            ull curDis=pq.top();
            auto &idList=*bundle.getIDList(curDis);
            pq.pop();
...
//                            }
                            ...
                            auto &piv= *bundle.getIDList(dis[v]);
                            piv.push_back(v);
                        }
