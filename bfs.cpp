...
struct ThreadData {
    ...
    ReservedVector<uint32> noHashIDList[MAX_NO_HASH_SIZE];
    unordered_map<uint16, ReservedVector<uint32>> disMap_16;
    unordered_map<uint32, ReservedVector<uint32>> disMap_32;
    unordered_map<uint64, ReservedVector<uint32>> disMap_64;

    inline ReservedVector<uint32> *getIDList_16(const uint16 &distance) {
    ...
    inline ReservedVector<uint32> *getIDList_32(const uint32 &distance) {
        if (distance < MAX_NO_HASH_SIZE) {
            return &noHashIDList[distance];
        } else {
    ...
    inline ReservedVector<uint32> *getIDList_64(const uint64 &distance) {
        if (distance < MAX_NO_HASH_SIZE) {
            return &noHashIDList[distance];
        } else {
...
int main(int argc, char *argv[]) {
    nice(-20);
    string testFile = "/home/data14.txt";
